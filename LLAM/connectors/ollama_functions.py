...
DEFAULT_SYSTEM_TEMPLATE_NOFC = """Reply to the user using last messages as context and do not repeat previous messages. 
...
You must always select one or more of the above tools and respond with only a JSON object matching the following schema:
...
DEFAULT_RESPONSE_FUNCTION = {
    "name": "__add_conversational_response",
    "description": (
        "Provide a text response for queries that don't match any of the tools or let the user know that the request being processed."
    ),
    "parameters": {
        "type": "object",
        "properties": {
...
class OllamaFunctions(ChatOllama):
    ...
    no_ollama_functions_template: str = DEFAULT_SYSTEM_TEMPLATE_NOFC
    ...
    def _generate(
        ...
        **kwargs: Any,
        ...
        if functions and _is_pydantic_class(functions[0].__class__):
            try:
                functions = [convert_to_ollama_tool(fn) for fn in functions if _is_pydantic_class(fn.__class__)]
            except Exception:
                pass
        
        elif functions and isinstance(functions[0], dict):
            try:
        ...
        if functions:
            ...
            no_ollama_functions_prompt_template = SystemMessagePromptTemplate.from_template(
                self.no_ollama_functions_template
            )

            system_message = no_ollama_functions_prompt_template.format()
        ...
        # Get calls
        calls = parsed_chat_result.get("tools", [])

        text_response = parsed_chat_result.get("response", "")

        if ((not functions and text_response) or 
            (functions and not calls and text_response)):
            return ChatResult(
                generations=[
                    ChatGeneration(
                        message=AIMessage(
        ...
        for call in calls:
            ...
            called_tool_name = call["tool"]
            called_tool_arguments = call["tool_input"]
            called_tool = next(
            ...
            if called_tool["name"] == DEFAULT_RESPONSE_FUNCTION["name"]:
                ...
                text_content = " ".join((text_content, called_tool_arguments["response"]))

            else:

                call_id = "call_" + str(uuid.uuid4()).replace('-', '')[:24]
                ...
                arg_schema = {**base_schema, "arguments": arguments, "type": "function"}
                tool_schema = {**base_schema, "args": called_tool_arguments}

                if not response_args:

                    response_args = {
                        "tool_calls": [arg_schema]
                    }

                    tool_calls = [tool_schema]

                else:
                    response_args["tool_calls"].append(arg_schema)
                    tool_calls.append(tool_schema)
            ...
            generations=[
                ChatGeneration(
                    message=AIMessage(
                        ...
                        additional_kwargs=response_args
                    )
                )
            ]
        )
